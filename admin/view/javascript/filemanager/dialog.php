...
?>
            ...
            ?>		<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<figcaption>
				    ...
				    <a class="tip-right preview" title="<?php echo trans('Preview')?>" data-url="<?php echo $src;?>" data-toggle="lightbox-1" href="javascript:responsive_filemanager_callback('<?php echo $current_path.$rfm_subfolder.$subdir.$file;?>');"><i class=" icon-eye-open"></i></a>
...
<script>
    ...
    <?php } ?>
<script>
function responsive_filemanager_callback(src) {
	
	var img_src = src.replace("../../../../image/","");
	var img_thumb = img_src.replace("catalog","cache");
	img_thumb = '<?php echo $_GET['server']; ?>image/'+img_thumb;
	
	//alert(img_src+" | "+img_thumb);
	
	<?php if ($_GET['thumb']) { ?>
	parent.$('#<?php echo $_GET['thumb']; ?>').find('img').attr('src', img_thumb);
	<?php } ?>
	
	<?php if ($_GET['target']) { ?>
	parent.$('#<?php echo $_GET['target']; ?>').attr('value', img_src);
	<?php } else { ?>
	var range, sel = parent.document.getSelection(); 
	img_src = '<?php echo $_GET['server']; ?>image/'+img_src;
