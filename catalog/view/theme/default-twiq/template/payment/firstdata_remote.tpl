    ...
    <div class="form-group">
      <div class="col-sm-12">
        ...
        {% for card in stored_cards %}
          <p><input type="radio" name="cc_choice" value="{{ card['token'] }}" class="stored_card" {{ i == 0 ? 'checked="checked"' : '' }}/> {{ card['card_type'] . ' xxxx ' . $card['digits'] . ', ' . $entry_cc_expire_date . ' ' . $card['expire_month'] . '/' . $card['expire_year'] }}</p>
          <?php $i++; ?>
        {% endif %}
